...
We build an end-to-end pipeline for multi-player tracking and short-horizon trajectory forecasting on SportsMOT basketball footage. SAM3.1 produces per-frame detections; a geometry-free augmentation layer (sanitize + velocity cap) stabilizes tracks before forecasting. We compare plain LSTM, rule-conditioned LSTM, graph LSTM, and a **residual LSTM** that predicts corrections over a constant-velocity linear baseline. On our primary clip (`sportsmot_example`, 12 temporal seeds at 2s spacing), the residual model **ties the linear baseline** on median rollout ADE (~5.8 px) while plain LSTM remains far worse (~10.7 px). Three seeds fail across all models due to SAM tracking collapse, not LSTM-specific error. Cross-sequence transfer to three additional basketball clips is **blocked** by a truncated local `sportsmot_publish.zip`; infrastructure and runbooks are in place for completion after re-download.
